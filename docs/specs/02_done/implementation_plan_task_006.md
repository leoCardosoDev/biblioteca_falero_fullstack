...
### Domain Layer
#### [MODIFY] [user.ts](../../../backend/src/domain/models/user.ts)
- Remove `role`, `status`.

#### [NEW] [login.ts](../../../backend/src/domain/models/login.ts)
- Define `LoginModel` with `id`, `userId`, `password`, `role`, `status`.

#### [MODIFY] [add-user.ts](../../../backend/src/domain/usecases/add-user.ts)
- Update params to exclude role/status.

#### [NEW] [add-user-login.ts](../../../backend/src/domain/usecases/add-user-login.ts)
- Define interface for creating logins.

### Infrastructure Layer (Persistence)
#### [MODIFY] [user-entity.ts](../../../backend/src/infra/db/typeorm/entities/user-entity.ts)
- Remove columns: `role`, `status`.

#### [NEW] [login-entity.ts](../../../backend/src/infra/db/typeorm/entities/login-entity.ts)
- Create Entity for `login` table.

#### [MODIFY] [user-repository.ts](../../../backend/src/infra/db/typeorm/user-repository.ts)
...
#### [NEW] [login-repository.ts](../../../backend/src/infra/db/typeorm/login-repository.ts)
- Implement `AddLoginRepository`, `LoadLoginByUserId`.

### Application Layer
#### [MODIFY] [db-add-user.ts](../../../backend/src/application/usecases/db-add-user.ts)
- Remove encryption/role logic. Just save User.

#### [NEW] [db-add-user-login.ts](../../../backend/src/application/usecases/db-add-user-login.ts)
- Handle password hashing and saving Login.

### Presentation Layer
#### [MODIFY] [user-routes.ts](../../../backend/src/main/routes/user-routes.ts)
- Update `POST /users` schema.
- Add `POST /users/:id/login`.
- Update `GET /users` schema to include nested `login`.

#### [MODIFY] [load-users-controller.ts](../../../backend/src/presentation/controllers/user/load-users-controller.ts)
